...
Sound* Sound::getInstance()
{
	if (_instance == nullptr)
		_instance = new Sound(GameGlobal::getInstance()->getWnd());
	return _instance;
}
...
Sound::Sound(HWND hWnd) : _isMute(false), _volume(75.0f)
{
	_primaryBuffer = nullptr;

	DSBUFFERDESC bufferDesc; //describe the buffer

	HRESULT result = DirectSoundCreate8(nullptr, &_pDevice, nullptr);
	if (result != S_OK)
		return;
	...
	result = _pDevice->CreateSoundBuffer(&bufferDesc, &_primaryBuffer, nullptr);
	if (result != S_OK)
		return;
...
Sound::~Sound()
{
	for (std::map<std::string, IDirectSoundBuffer8*>::iterator it = _soundBufferMap.begin(); it != _soundBufferMap.end()
	     ; ++it)
	{
		if (it->second)
		{
...
void Sound::loadSound(char* fileName, const std::string& name)
{
	...
	_pDevice->CreateSoundBuffer(&bufferDesc, &tempBuffer, nullptr);
	...
	result = (*pSecondaryBuffer)->Lock(0, waveHeaderStruct.dataSize, reinterpret_cast<void**>(&bufferPtr),
	                                   static_cast<DWORD*>(&bufferSize), nullptr, nullptr, 0);
