...
void Player::ChangeAnimation(StateName state)
{
	switch (state)
	{
	case APPEAR:
...
void Player::handlerKeyBoard(const std::map<int, bool>& keys, float dt)
{
	if (!_islock)
	{
		if (_playerData->state)
			_playerData->state->handlerKeyBoard(keys, dt);
	}
	_islock = false;
}
...
void Player::Update(float dt)
{
	...
	if (_alive)
	{
		...
		{
			int TimePause = _timeAlive / 0.05f;//0.1 time twinkle
			if (TimePause % 2 == 0)
				_isAllowDraw = true;
...
void Player::PlayerShoot(PlayerBullet::BulletType bulletType)
{
