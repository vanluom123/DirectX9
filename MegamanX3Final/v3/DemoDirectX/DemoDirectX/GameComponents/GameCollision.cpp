...
float GameCollision::SweptAABB(const RECT& obj, const RECT& other, const GVec2& distance, BaseObject::eSideCollision& sideCollision)
{
	...
	{
		sideCollision = BaseObject::SIDE_COLLISION_NONE;
		return 1.0f;
	}
	...
	if (txEntry < tyEntry)
	{
		if (dyEntry < 0.0f)
		{
			sideCollision = BaseObject::SIDE_COLLISION_TOP;
		}
		else
		{
			sideCollision = BaseObject::SIDE_COLLISION_BOTTOM;
		}
	}
	else
	{
		if (dxEntry < 0.0f)
		{
			sideCollision = BaseObject::SIDE_COLLISION_LEFT;
		}
		else
		{
			sideCollision = BaseObject::SIDE_COLLISION_RIGHT;
		}
	}
...
BaseObject::eSideCollision GameCollision::SetSideCollision(BaseObject* e1, BaseObject* e2)
{
	...
	if (abs(int(dx)) <= w && abs(int(dy)) <= h)
	{
		...
		if (wy > hx)
		{
			...
			{
				//Collision Top e1
				return BaseObject::SIDE_COLLISION_TOP;
			}
			//Collision Right e1
			return BaseObject::SIDE_COLLISION_RIGHT;
		}
		if (wy > -hx)
		{
			//Collision Left e1
			return BaseObject::SIDE_COLLISION_LEFT;
		}

		//Collision Bottom e1
		return BaseObject::SIDE_COLLISION_BOTTOM;
	}
	//Don't collide
	return BaseObject::SIDE_COLLISION_NONE;
}
...
BaseObject::eSideCollision GameCollision::SetSideCollision(BaseObject* e1, BaseObject::CollisionReturn data)
{
	...
	*/
	...
	if (vec.y < 0)
	{
		...
		if (vec.x > 0.35f && vec.x < 0.8f)
		{
			...
			return BaseObject::SIDE_COLLISION_TOP_RIGHT;
		}
		if (vec.x >= 0.8f)
		{
			return BaseObject::SIDE_COLLISION_RIGHT;
		...
		{
			//Top - Left collision
			return BaseObject::SIDE_COLLISION_TOP_LEFT;
		}
	...
	*/
	...
	if (vec.x > 0.35 && vec.x < 0.8)
	{
		...
		return BaseObject::SIDE_COLLISION_BOTTOM_RIGHT;
	}
	if (vec.x >= 0.8)
	{
		return BaseObject::SIDE_COLLISION_RIGHT;
	...
	if (vec.x < -0.35f && vec.x > -0.8f)
	{
		...
		return BaseObject::SIDE_COLLISION_BOTTOM_LEFT;
	}
