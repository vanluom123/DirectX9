...
Entity::CollisionReturn GameCollision::RectAndRect(RECT rect1, RECT rect2)
{
	...
	// MaxLeft
	result.RegionCollision.left = ValueMax(rect1.left, rect2.left);
	// MaxRight
	result.RegionCollision.right = ValueMax(rect1.right, rect2.right);
	// MaxTop
	result.RegionCollision.top = ValueMax(rect1.top, rect2.top);
	// MaxBottom
	result.RegionCollision.bottom = ValueMax(rect1.bottom, rect2.bottom);
...
bool GameCollision::PointAndRect(float x, float y, RECT rect)
{
	return !(x < rect.left || x > rect.right || y < rect.top || y > rect.bottom);
}

bool GameCollision::RectAndCircle(RECT rect, int circleX, int circleY, int circleRadius)
{
	int px = circleX;
	int py = circleY;
