...
bool Input::CreateInputDevice(HWND hWnd)
{
	...
	if (FAILED(result))
	{
		MessageBox(hWnd, "Create input device failed", "Error", MB_OK);
		return false;
	}

	if (!CreateKeyboard(hWnd))
	{
		MessageBox(hWnd, "Create keyboard device failed", "Error", MB_OK);
		return false;
	}

	if (!CreateMouse(hWnd))
	{
		MessageBox(hWnd, "Create mouse device failed", "Error", MB_OK);
		return false;
	}
...
{
	HRESULT result = m_Input->CreateDevice(GUID_SysKeyboard, &m_Keyboard, NULL);
	if (FAILED(result))
		return false;
...
{
	HRESULT result = m_Input->CreateDevice(GUID_SysMouse, &m_Mouse, NULL);
	if (FAILED(result))
		return false;
