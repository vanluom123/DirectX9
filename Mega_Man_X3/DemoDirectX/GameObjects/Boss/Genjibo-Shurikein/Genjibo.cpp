...
Genjibo::Genjibo()
{
	...
	animDie = new Animation(Define::EXPLOSIONS, 1, 8, 35, 30, 0.05f);
	sideGen = eSide_None;
	change = false;
...
void Genjibo::setState(Shurikein_State keinState)
{
	...
	switch (shurikeinState)
	{
		case eShurikein_Appear:
		{
			_vx = 0.0f;
			_vy = 150.0f;
			anim->setAnimation(1, 10, 0.05f);
			animGenjinbo->setAnimation(0, 2, 0.05f);
			posY = 920.0f;
			break;
		}
		case eShurikein_Stand:
		...
		case eShurikein_Attack_1:
		{
			...
			anim->setAnimation(5, 10, 0.03f);
			break;
		}
		case eShurikein_Attack_2:
		{
			currentState = eGenjibo_Move;
			anim->setAnimation(6, 10, 0.03f);
			_vx = -150.0f;
			_vy = 150.0f;
			break;
		}
		...
		{
			currentState = eGenjibo_Move;
			anim->setAnimation(7, 17, 0.01f);
			_vx = -80.0f;
			_vy = 150.0f;
			break;
		}
		case eShurikein_Death:
			animDie->setAnimation(0, 8, 0.05f, false);
			_vx = 0.0f;
			_vy = 0.0f;
			break;
		default:
	...
	switch (shurikeinState)
	{
		case eShurikein_Appear:
		{
			if (posY >= _posY - 8)
			{
				...
				{
					animGenjinbo->setAnimation(1, 2, 0.05f);
					_isAllowDraw = true;
				}
				timeMove += dt;
				if (timeMove > 2.0f)
